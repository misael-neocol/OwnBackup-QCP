...
export function onBeforeCalculate(quote, lines, conn) {
    ...
    return new Promise((resolve, reject) => {
        ...
        //Call the code to cascade the lines
        cascadeFields(quote, lines);
        cascadeAdditionalDiscount(quote, lines);

        //setDiscount(quote, lines);

        setTargetOneTimeAmount(quote, lines);
		removeOneTimeDiscount(quote);
        setGroupDates(quote);
        clearValidation(quote);
		//rollUpActualOneTimeDiscount(quote, lines);
        //setRequiredByDiscount(lines);
        
        //clearDiscount(quote, lines);
...
function cascadeFields(quote, lines){
    ...
    cascadeLicenseType(quote, lines);
    //cascadeAdditionalDiscount(quote, lines);
    cascadeTargetCustomerAmount(quote, lines);
}
...
function cascadeQuantity(quote, lines){
    ...
    bundleLines.forEach(bundle => {
        ...
        bundle.components.forEach(component => {
            ...
            /*if( currentQuantity < quoteMinimumQuantity && component.record.Minimum_Users__c){
                component.record.SBQQ__Quantity__c = quoteMinimumQuantity ? quoteMinimumQuantity : currentQuantity;
            }*/
